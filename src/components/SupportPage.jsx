...
const TicketModal = ({ isOpen, onClose, ticket, onSaved, drivers }) => {
  ...
  useEffect(() => {
    ...
    if (ticket) {
      ...
      // normalize issue_type: API may return "driver late" (space) → "driver_late"
      const rawType = (ticket.issue_type||"").replace(/\s+/g,"_");
      setIssueType(rawType); setPriority(ticket.priority||"");
      setDescription(ticket.description||""); setStatus(ticket.status||"open");
    } else {
      setDriverId(""); setTripId(""); setIssueType(""); setPriority(""); setDescription(""); setStatus("open");
    }
    setAttachment(null); setDriverTrips([]);
  }, [isOpen, ticket?.id]);

  // جيب رحلات السائق لما يتغير
  useEffect(() => {
    if (!driverId) { setDriverTrips([]); return; }
    setLoadingTrips(true);
    fetch(`${BASE}/trips?driver_id=${driverId}`)
      .then(r => r.json())
      .then(d => { setDriverTrips(Array.isArray(d) ? d : []); setLoadingTrips(false); })
      .catch(() => { setDriverTrips([]); setLoadingTrips(false); });
  }, [driverId]);
  ...
  const submit = async e => {
    ...
    // timeout after 5s — server closes connection on PUT without body
    const timer = setTimeout(() => controller.abort(), 5000);
    try {
      await fetch(isEditing ? `${BASE}/tickets/${ticket.id}` : `${BASE}/tickets`, {
        method: isEditing ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ driver_id: driverId, trip_id: tripId || null, issue_type: issueType, priority, description, status }),
        signal: controller.signal,
      });
    } catch (_) {
      // AbortError (timeout) or connection-close = server processed it
    } finally {
      clearTimeout(timer);
      setSaving(false);
    }
          ...
          <TicketField label="الرحلة المرتبطة (اختياري)">
            <TicketSelWrap>
              <select className={selCls} value={tripId} onChange={e=>setTripId(e.target.value)} disabled={!driverId || loadingTrips}>
                <option value="">{loadingTrips ? "جارٍ التحميل..." : driverId ? (driverTrips.length===0 ? "لا توجد رحلات" : "اختر الرحلة") : "اختر السائق أولاً"}</option>
                {driverTrips.map(t=>(
                  <option key={t.id} value={t.id}>
                    {t.id} — {t.from||""} {t.to ? `← ${t.to}` : ""}
                  </option>
                ))}
              </select>
            </TicketSelWrap>
...
export default function SupportPage() {
  ...
  const fetchTickets = () => {
    ...
    fetch(`${BASE}/tickets`)
      .then(r => r.json())
      .then(d => { setTickets(Array.isArray(d.tickets) ? d.tickets : []); setLoading(false); })
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`${BASE}/tickets/${ticketId}`, { method: "DELETE" });
    } catch(_) {
      // server closes connection without body — treat as success
    }
    // always remove from list and close modal
